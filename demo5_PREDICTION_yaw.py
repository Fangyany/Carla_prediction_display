...
def create_trajectory_tensor(history_trajectory, surrounding_vehicles_queues):
    ...
    # 'x', 'y', 'frame_exists', 'yaw', 'speed', 'traffic_flag', 'traffic_state'
    target_traj += [(*target_traj[-1][:2], 0, 0, 0, 0, -1)] * (20 - len(target_traj))
    trajectories.append(target_traj)

    # 添加周围车辆的轨迹
    for vehicle_id, queue in surrounding_vehicles_queues.items():
        vehicle_traj = list(queue)
        vehicle_traj += [(*vehicle_traj[-1][:2], 0, 0, 0, 0, -1)] * (20 - len(vehicle_traj))
...
try:
    while True:
        ...
        # world.debug.draw_string(location=target_vehicle.get_location(), text='yaw: {:.2f}'.format(target_vehicle.get_transform().rotation.yaw), color=carla.Color(r=255, g=0, b=0), life_time=10)
